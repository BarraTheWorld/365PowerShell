...
function New-VMBuildAzureRM2
{
    ...
    (
        ...
        [validateSet('PSObject','Contoso')]
        [String]$KeyVault = 'Contoso',
        ...
        [ValidateSet('ExtraSmall','Small','Medium','Large','Graphics')]
        ...
        # Choose the StorageType that you wish to add the VirtualMachine
        [ValidateSet('PremiumLRS','StandardLRS')]
        [String]$StorageType = 'StandardLRS',

        # Choose the Windows Image for the VirtualMachine
        [ValidateSet('2008-R2-SP1','2008-R2-SP1-smalldisk',
                     '2012-Datacenter','2012-Datacenter-smalldisk','2012-R2-Datacenter','2012-R2-Datacenter-smalldisk',
                     '2016-Datacenter','2016-Datacenter-Server-Core','2016-Datacenter-Server-Core-smalldisk',
                     '2016-Datacenter-smalldisk','2016-Datacenter-with-Containers','2016-Datacenter-with-Containers-smalldisk','2016-Datacenter-with-RDSH','2016-Nano-Server')]
        [String]$WindowsImage = '2016-Datacenter-with-Containers',
    ...
    {   
        ...
        $MySKU = Get-AzureRmVMImageSku  -Location $Location -PublisherName MicrosoftWindowsServer -Offer WindowsServer | 
                   Where Skus -EQ $WindowsImage | Foreach Skus

        #endregion
        #region OS Configuration settings
        try {
            # Maintain a keyvault per region with the required Credentials
            # Also use the same keyvault for Certs for winrm listener
            # Setup keyvaults for different types of deployments, Contoso is default

            $KV = Get-AzureRmKeyVault -ResourceGroupName rgglobal -ErrorAction Stop | where { $_.location -eq $Location -and $_.VaultName -like "*$KeyVault*" }
            $Cert= Get-AzureKeyVaultCertificate -VaultName $KV.VaultName | where { $_.Name -like "*$KeyVault*wildcard*" }
            $Cert= Get-AzureKeyVaultCertificate -VaultName $KV.VaultName -Name $Cert.Name
            $SS= Get-AzureKeyVaultSecret -VaultName $KV.VaultName -Name $LocalAdminUser -ErrorAction Stop
...
'@
        ...
        $OSConfiguration = @{
            ...
            Credential          = $Cred
            TimeZone            = $TimeZone
            WinRMCertificateUrl = $cert.SecretId
            WinRMHttps          = $true
            WinRMHttp           = $true
            }
        #endregion
    ...
    Process
    {
        $VMName | ForEach-Object {
            ...
            if ($AvailabilitySetName)
            {
                ...
                Catch {
                    $AvailabilitySet = New-AzureRmAvailabilitySet -Sku Aligned -PlatformUpdateDomainCount $VMName.count `
            ...
            $MyVM = New-AzureRMVMConfig @VMConfig  | 
                Set-AzureRmVMSourceImage -PublisherName MicrosoftWindowsServer -Offer WindowsServer -Skus  $MySKU -Version latest |
                Set-AzureRmVMOperatingSystem @OSConfiguration |
                Set-AzureRmVMOSDisk -Name ($VM + "_OSDisk") -Windows -StorageAccountType $StorageType -CreateOption FromImage |
                Add-AzureRmVMNetworkInterface -Id $Interface.Id |
                Add-AzureRmVMSecret -SourceVaultId $KV.ResourceId -CertificateStore My -CertificateUrl $cert.SecretId  |
                Add-AzureRmVMSecret -SourceVaultId $KV.ResourceId -CertificateStore Root -CertificateUrl $cert.SecretId  |
                Add-AzureRmVMSecret -SourceVaultId $KV.ResourceId -CertificateStore CA -CertificateUrl $cert.SecretId  |
                # Set-AzureRmVMDiagnosticsExtension
                # Set-AzureRmVMBootDiagnostics
                # Set-AzureRmVMAccessExtension
                Add-AzureRmVMDataDisk -Name ($VM + "_DataDisk") -StorageAccountType $StorageType -Caching ReadWrite `
